	...
	public class PlayerSteering
	{
			...
			out Vector3 moveVec)
		{
			...
			foreach(Input.InputAction act in actions)
			{
				if(act.mAction.Equals(mMoveLeftRight))
				{
					...
					actionMult	+=Math.Abs(act.mTimeHeldMS);
					multCount++;
				}
				...
				else if(act.mAction.Equals(mMoveForwardFast))
				{
					mbMovedThisFrame	=true;
					moveVec				+=camForward;
					actionMult			+=act.mTimeHeldMS * SprintForwardFactor;
					multCount++;
				}
				else if(act.mAction.Equals(mMoveBackFast))
				{
					mbMovedThisFrame	=true;
					moveVec				-=camForward;
					actionMult			+=act.mTimeHeldMS * SprintBackFactor;
					multCount++;
				}
				else if(act.mAction.Equals(mMoveLeftFast))
				{
					mbMovedThisFrame	=true;
					moveVec				+=camLeft;
					actionMult			+=act.mTimeHeldMS * SprintLeftFactor;
					multCount++;
				}
				else if(act.mAction.Equals(mMoveRightFast))
				{
					mbMovedThisFrame	=true;
					moveVec				-=camLeft;
					actionMult			+=act.mTimeHeldMS * SprintRightFactor;
					multCount++;
				}
			}
		...
		void GetTurn(List<Input.InputAction> actions)
		{
			foreach(Input.InputAction act in actions)
			{
				...
				{
					float	pitchAmount	=act.mTimeHeldMS * 0.4f;
				...
				{
					float	delta	=act.mTimeHeldMS * 0.4f;
					mYaw	-=delta;
				}
